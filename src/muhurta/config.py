...
def _parse_tithi_list(raw: list[str], context: str) -> tuple[Tithi, ...]:
    ...
    for item in raw:
        try:
            ...
            raise InvalidMuhurtaConfigError(f"{context}: invalid tithi {item!r}") from exc
    return tuple(result)
...
def _parse_karana_list(raw: list[str], context: str) -> tuple[Karana, ...]:
    ...
    for item in raw:
        try:
            ...
            raise InvalidMuhurtaConfigError(f"{context}: invalid karana {item!r}") from exc
    return tuple(result)
...
def _parse_yoga_list(raw: list[str], context: str) -> tuple[Yoga, ...]:
    ...
    for item in raw:
        try:
            ...
            raise InvalidMuhurtaConfigError(f"{context}: invalid yoga {item!r}") from exc
    return tuple(result)
...
def _parse_nakshatra_list(raw: list[str], context: str) -> tuple[NakshatraId, ...]:
    ...
    for item in raw:
        try:
            ...
            raise InvalidMuhurtaConfigError(f"{context}: invalid nakshatra {item!r}") from exc
    return tuple(result)
...
def _parse_var_list(raw: list[str], context: str) -> tuple[Var, ...]:
    ...
    for item in raw:
        try:
            ...
            raise InvalidMuhurtaConfigError(f"{context}: invalid vara {item!r}") from exc
    return tuple(result)
...
def _parse_category_rule(
    ...
    required_nak = _parse_nakshatra_list(list(raw_nak), ctx) if isinstance(raw_nak, list) else ()

    raw_tithis = raw.get("avoided_tithis", [])
    avoided_tithis = (
        _parse_tithi_list(list(raw_tithis), ctx) if isinstance(raw_tithis, list) else ()
    )

    raw_karanas = raw.get("avoided_karanas", [])
    avoided_karanas = (
        _parse_karana_list(list(raw_karanas), ctx) if isinstance(raw_karanas, list) else ()
    )

    raw_yogas = raw.get("avoided_yogas", [])
    avoided_yogas = _parse_yoga_list(list(raw_yogas), ctx) if isinstance(raw_yogas, list) else ()

    raw_vars = raw.get("avoided_vars", [])
    avoided_vars = _parse_var_list(list(raw_vars), ctx) if isinstance(raw_vars, list) else ()

    raw_pref = raw.get("preferred_vars", [])
    preferred_vars = _parse_var_list(list(raw_pref), ctx) if isinstance(raw_pref, list) else ()
...
def load_config(path: str | Path | None = None) -> MuhurtaConfig:
    ...
    if not isinstance(section, dict):
        raise InvalidMuhurtaConfigError(f"[muhurta] section must be a table, got {section!r}")
    ...
    if not isinstance(version, str) or version == "":
        raise InvalidMuhurtaConfigError(f"version must be a non-empty string, got {version!r}")
