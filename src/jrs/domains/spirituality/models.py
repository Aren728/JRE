"""Spirituality domain data models — outcome taxonomy, rule catalog, fact evaluation."""
...
class SpiritualityOutcomeTaxonomy(Enum):
    """Highly specific spirituality/renunciation outcome taxonomies."""

    SPIRITUAL_AWAKENING = "SPIRITUAL_AWAKENING"
    RENUNCIATION = "RENUNCIATION"
    ASCETIC_TENDENCIES = "ASCETIC_TENDENCIES"
    RELIGIOUS_DEVOTION = "RELIGIOUS_DEVOTION"
    OCCULT_INTEREST = "OCCULT_INTEREST"
    MOKSHA_INDICATORS = "MOKSHA_INDICATORS"
...
class SpiritualityRule:
    """A single classical rule mapping JRE facts to a spirituality outcome."""
...
@dataclass(frozen=True)
class SpiritualityRuleCatalog:
    """Complete catalog of spirituality domain rules."""

    rules: tuple[SpiritualityRule, ...] = ()
    ...
    def get_rule_by_id(self, rule_id: str) -> SpiritualityRule | None:
...
@dataclass(frozen=True)
class SpiritualityConfig:
    """Configuration for the spirituality domain."""
...
def evaluate_rule(
    ...
    Args:
        rule: The spirituality rule to evaluate.
...
def evaluate_facts(
    ...
    Args:
        rules: The tuple of spirituality rules to evaluate.
        facts: The facts dictionary from JRE engines.
