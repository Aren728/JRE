"""Litigation/Conflict domain data models — outcome taxonomy, rule catalog, fact evaluation."""
...
class LitigationOutcomeTaxonomy(Enum):
    """Highly specific litigation/conflict outcome taxonomies."""

    LEGAL_VICTORY = "LEGAL_VICTORY"
    LEGAL_DEFEAT = "LEGAL_DEFEAT"
    PROLONGED_LITIGATION = "PROLONGED_LITIGATION"
    SETTLEMENT_OUT_OF_COURT = "SETTLEMENT_OUT_OF_COURT"
...
class LitigationRule:
    """A single classical rule mapping JRE facts to a litigation outcome."""
...
@dataclass(frozen=True)
class LitigationRuleCatalog:
    """Complete catalog of litigation domain rules."""

    rules: tuple[LitigationRule, ...] = ()
    ...
    def get_rule_by_id(self, rule_id: str) -> LitigationRule | None:
...
class LitigationConfig:
    """Configuration for the litigation domain."""
...
def evaluate_rule(
    ...
    Args:
        rule: The litigation rule to evaluate.
...
def evaluate_facts(
    ...
    Args:
        rules: The tuple of litigation rules to evaluate.
        facts: The facts dictionary from JRE engines.
