"""Litigation domain configuration loader."""
...
from .errors import InvalidLitigationConfigError
from .models import LitigationConfig, LitigationOutcomeTaxonomy, LitigationRule
...
_CONFIG_PATH = (
    ...
    / "config" / "domains" / "litigation.toml"
)


def load_litigation_config(path: Path | None = None) -> LitigationConfig:
    """Load and validate litigation domain configuration from a TOML file.

    Args:
        path: Path to the TOML config file.

    Returns:
        A validated ``LitigationConfig`` instance.

    Raises:
        InvalidLitigationConfigError: If the config is invalid.
    """
    config_path = path or _CONFIG_PATH
    if not config_path.exists():
        raise InvalidLitigationConfigError(
            f"Litigation config not found: {config_path}",
        )
    ...
    try:
        ...
        raise InvalidLitigationConfigError(f"Invalid TOML: {exc}") from exc
...
def load_litigation_rules(path: Path | None = None) -> tuple[LitigationRule, ...]:
    """Load litigation rules from the TOML config.

    Args:
        path: Path to the TOML config file.

    Returns:
        A tuple of LitigationRule objects.

    Raises:
        InvalidLitigationConfigError: If the config or rules are invalid.
    """
    config_path = path or _CONFIG_PATH
    if not config_path.exists():
        raise InvalidLitigationConfigError(
    ...
    if not isinstance(rules_raw, list):
        raise InvalidLitigationConfigError("rules must be a list")
...
def _parse_rule(data: dict[str, Any]) -> LitigationRule:
    """Parse a single rule from TOML data."""
    rule_id = data.get("rule_id", "")
    if not rule_id:
        raise InvalidLitigationConfigError("rule_id must not be empty")
    ...
    try:
        outcome = LitigationOutcomeTaxonomy(outcome_str)
