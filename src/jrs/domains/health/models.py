"""Health/Vitality domain data models — outcome taxonomy, rule catalog, fact evaluation.
...
def _validate_no_medical_terms(text: str) -> None:
    """Raise an error if medical terminology is detected in text.

    This is a safety gate that prevents any medical diagnosis language
    from entering the system through rule descriptions or fact types.

    Args:
        text: The text to validate.

    Raises:
        ValueError: If forbidden medical terms are detected.
    """
    text_lower = text.lower()
    for term in _FORBIDDEN_TERMS:
        if term in text_lower:
            raise ValueError(
                f"Forbidden medical term '{term}' detected in: '{text}'. "
                "Health domain uses only traditional constitutional vitality "
                "indicators. No medical diagnosis language permitted."
            )
...
class HealthOutcomeTaxonomy(Enum):
    """Traditional astrological vitality and constitution indicators.

    These taxonomies represent classical Jyotish assessments of physical
    constitution and life-force patterns. They do NOT correspond to any
    medical diagnostic categories.
    """

    HIGH_VITALITY = "HIGH_VITALITY"
    LOW_VITALITY = "LOW_VITALITY"
    CHRONIC_STRESS = "CHRONIC_STRESS"
    STRONG_RECOVERY_CAPACITY = "STRONG_RECOVERY_CAPACITY"
    ENERGY_FLUCTUATIONS = "ENERGY_FLUCTUATIONS"
    TRADITIONAL_CONSTITUTION_INDICATORS = "TRADITIONAL_CONSTITUTION_INDICATORS"
...
class HealthRule:
    """A single classical rule mapping JRE facts to a vitality outcome.

    SAFETY CONSTRAINT: The description and supporting_fact_type fields
    must never contain medical diagnosis terminology.
    """
    ...
    def __post_init__(self) -> None:
        """Validate that no medical terminology is present."""
        _validate_no_medical_terms(self.description)
        for fact in self.condition_facts:
            _validate_no_medical_terms(fact)
...
class HealthRuleCatalog:
    """Complete catalog of health domain rules."""
    ...
    def get_rule_by_id(self, rule_id: str) -> HealthRule | None:
...
def evaluate_rule(
    ...
    SAFETY CONSTRAINT: The resulting EvidenceRecord outcome_taxonomy
    will always be a traditional vitality indicator, never a medical term.

    Args:
        rule: The health rule to evaluate.
...
def evaluate_facts(
    ...
    Args:
        rules: The tuple of health rules to evaluate.
        facts: The facts dictionary from JRE engines.
