"""Traits domain data models — outcome taxonomy, rule catalog, fact evaluation.

This is a JRS (interpretation) domain that consumes deterministic facts
from JRE-027 (BirthSignature) and outputs trait assessments.
"""
...
class TraitOutcomeTaxonomy(Enum):
    """Trait outcome categories derived from BirthSignature facts."""

    INTELLECTUAL_DEPTH = "INTELLECTUAL_DEPTH"
    EMOTIONAL_VOLATILITY = "EMOTIONAL_VOLATILITY"
    PRACTICAL_GROUNDEDNESS = "PRACTICAL_GROUNDEDNESS"
...
class TraitRule:
    """A single classical rule mapping JRE-027 facts to a trait outcome."""
...
@dataclass(frozen=True)
class TraitRuleCatalog:
    """Complete catalog of traits domain rules."""

    rules: tuple[TraitRule, ...] = ()
    ...
    def get_rule_by_id(self, rule_id: str) -> TraitRule | None:
...
@dataclass(frozen=True)
class TraitsConfig:
    """Configuration for the traits domain."""
...
def evaluate_rule(
    ...
    Args:
        rule: The trait rule to evaluate.
        facts: The facts dictionary from JRE-027.
...
def evaluate_facts(
    ...
    Args:
        rules: The tuple of trait rules to evaluate.
        facts: The facts dictionary from JRE-027.
