"""Traits domain configuration loader."""
...
from .errors import InvalidTraitsConfigError
from .models import TraitOutcomeTaxonomy, TraitRule, TraitsConfig
...
def load_traits_config(path: Path | None = None) -> TraitsConfig:
    """Load and validate traits domain configuration from a TOML file.

    Args:
        path: Path to the TOML config file.

    Returns:
        A validated ``TraitsConfig`` instance.

    Raises:
        InvalidTraitsConfigError: If the config is invalid.
    """
    config_path = path or _CONFIG_PATH
    if not config_path.exists():
        raise InvalidTraitsConfigError(
            f"Traits config not found: {config_path}",
        )
    ...
    try:
        ...
        raise InvalidTraitsConfigError(f"Invalid TOML: {exc}") from exc

    section = raw.get("traits")
    if not isinstance(section, dict):
        raise InvalidTraitsConfigError(
            "Missing top-level [traits] section",
        )
...
def load_traits_rules(path: Path | None = None) -> tuple[TraitRule, ...]:
    """Load traits rules from the TOML config.

    Args:
        path: Path to the TOML config file.

    Returns:
        A tuple of TraitRule objects.

    Raises:
        InvalidTraitsConfigError: If the config or rules are invalid.
    """
    config_path = path or _CONFIG_PATH
    if not config_path.exists():
        raise InvalidTraitsConfigError(
    ...
    if not isinstance(rules_raw, list):
        raise InvalidTraitsConfigError("rules must be a list")
...
def _parse_rule(data: dict[str, Any]) -> TraitRule:
    """Parse a single rule from TOML data."""
    rule_id = data.get("rule_id", "")
    if not rule_id:
        raise InvalidTraitsConfigError("rule_id must not be empty")
    ...
    try:
        outcome = TraitOutcomeTaxonomy(outcome_str)
