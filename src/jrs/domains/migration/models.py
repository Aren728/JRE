"""Migration domain data models — outcome taxonomy, rule catalog, fact evaluation."""
...
class MigrationOutcomeTaxonomy(Enum):
    """Highly specific foreign travel/migration outcome taxonomies."""

    FOREIGN_SETTLEMENT = "FOREIGN_SETTLEMENT"
    SHORT_TERM_TRAVEL = "SHORT_TERM_TRAVEL"
    MIGRATION_DELAY = "MIGRATION_DELAY"
    RETURN_TO_HOMELAND = "RETURN_TO_HOMELAND"
    CROSS_CULTURAL_SUCCESS = "CROSS_CULTURAL_SUCCESS"
    VISA_OBSTACLES = "VISA_OBSTACLES"
...
class MigrationRule:
    """A single classical rule mapping JRE facts to a migration outcome."""
...
@dataclass(frozen=True)
class MigrationRuleCatalog:
    """Complete catalog of migration domain rules."""
    ...
    def get_rule_by_id(self, rule_id: str) -> MigrationRule | None:
...
class MigrationConfig:
    """Configuration for the migration domain."""
...
def evaluate_rule(
    ...
    Args:
        rule: The migration rule to evaluate.
...
def evaluate_facts(
    ...
    Args:
        rules: The tuple of migration rules to evaluate.
        facts: The facts dictionary from JRE engines.
