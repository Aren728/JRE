"""Assets/Vehicles domain data models — outcome taxonomy, rule catalog, fact evaluation."""
...
class AssetsOutcomeTaxonomy(Enum):
    """Highly specific assets/vehicles outcome taxonomies."""
...
class AssetsRule:
    """A single classical rule mapping JRE facts to an assets outcome."""
...
@dataclass(frozen=True)
class AssetsRuleCatalog:
    """Complete catalog of assets domain rules."""

    rules: tuple[AssetsRule, ...] = ()
    ...
    def get_rule_by_id(self, rule_id: str) -> AssetsRule | None:
...
@dataclass(frozen=True)
class AssetsConfig:
    """Configuration for the assets domain."""
...
def evaluate_rule(
    ...
    Args:
        rule: The assets rule to evaluate.
...
def evaluate_facts(
    ...
    Args:
        rules: The tuple of assets rules to evaluate.
        facts: The facts dictionary from JRE engines.
