"""Career domain data models — outcome taxonomy, rule catalog, fact evaluation."""
...
class CareerOutcomeTaxonomy(Enum):
    ...
    CAREER_ASCENT = "CAREER_ASCENT"
    PROFESSIONAL_STAGNATION = "PROFESSIONAL_STAGNATION"
    CHANGE_OF_PROFESSION = "CHANGE_OF_PROFESSION"
    AUTHORITY_STATUS = "AUTHORITY_STATUS"
    LOSS_OF_EMPLOYMENT = "LOSS_OF_EMPLOYMENT"
    SUCCESSFUL_BUSINESS = "SUCCESSFUL_BUSINESS"
    GOVERNMENT_SERVICE = "GOVERNMENT_SERVICE"
    FOREIGN_CAREER = "FOREIGN_CAREER"
    CAREER_OBSTACLES = "CAREER_OBSTACLES"
    RECOGNITION_AWARDS = "RECOGNITION_AWARDS"
...
class CareerRule:
    """A single classical rule mapping JRE facts to a career outcome."""
...
@dataclass(frozen=True)
class CareerRuleCatalog:
    """Complete catalog of career domain rules."""

    rules: tuple[CareerRule, ...] = ()
    ...
    def get_rule_by_id(self, rule_id: str) -> CareerRule | None:
...
@dataclass(frozen=True)
class CareerConfig:
    """Configuration for the career domain."""
...
def evaluate_rule(
    ...
    Args:
        rule: The career rule to evaluate.
...
def evaluate_facts(
    ...
    Args:
        rules: The tuple of career rules to evaluate.
        facts: The facts dictionary from JRE engines.
