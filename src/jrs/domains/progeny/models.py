"""Progeny domain data models — outcome taxonomy, rule catalog, fact evaluation."""
...
class ProgenyOutcomeTaxonomy(Enum):
    """Highly specific children/progeny outcome taxonomies."""

    EASY_CONCEPTION = "EASY_CONCEPTION"
    DELAYED_PROGENY = "DELAYED_PROGENY"
    MULTIPLE_CHILDREN = "MULTIPLE_CHILDREN"
    CHALLENGES_WITH_CHILDREN = "CHALLENGES_WITH_CHILDREN"
    CHILDREN_SUCCESS = "CHILDREN_SUCCESS"
...
class ProgenyRule:
    """A single classical rule mapping JRE facts to a progeny outcome."""
...
@dataclass(frozen=True)
class ProgenyRuleCatalog:
    """Complete catalog of progeny domain rules."""

    rules: tuple[ProgenyRule, ...] = ()
    ...
    def get_rule_by_id(self, rule_id: str) -> ProgenyRule | None:
...
@dataclass(frozen=True)
class ProgenyConfig:
    """Configuration for the progeny domain."""
...
def evaluate_rule(
    ...
    Args:
        rule: The progeny rule to evaluate.
...
def evaluate_facts(
    ...
    Args:
        rules: The tuple of progeny rules to evaluate.
        facts: The facts dictionary from JRE engines.
