"""Business domain configuration loader."""
...
from .errors import InvalidBusinessConfigError
from .models import BusinessConfig, BusinessOutcomeTaxonomy, BusinessRule
...
def load_business_config(path: Path | None = None) -> BusinessConfig:
    """Load and validate business domain configuration from a TOML file.
    ...
    Raises:
        InvalidBusinessConfigError: If the config is invalid.
    """
    config_path = path or _CONFIG_PATH
    if not config_path.exists():
        raise InvalidBusinessConfigError(
    ...
    try:
        ...
        raise InvalidBusinessConfigError(f"Invalid TOML: {exc}") from exc
...
def load_business_rules(path: Path | None = None) -> tuple[BusinessRule, ...]:
    """Load business rules from the TOML config.

    Args:
        path: Path to the TOML config file.

    Returns:
        A tuple of BusinessRule objects.

    Raises:
        InvalidBusinessConfigError: If the config or rules are invalid.
    """
    config_path = path or _CONFIG_PATH
    if not config_path.exists():
        raise InvalidBusinessConfigError(
    ...
    if not isinstance(rules_raw, list):
        raise InvalidBusinessConfigError("rules must be a list")
...
def _parse_rule(data: dict[str, Any]) -> BusinessRule:
    """Parse a single rule from TOML data."""
    rule_id = data.get("rule_id", "")
    if not rule_id:
        raise InvalidBusinessConfigError("rule_id must not be empty")
    ...
    try:
        outcome = BusinessOutcomeTaxonomy(outcome_str)
