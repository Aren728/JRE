"""Integration tests for Property domain validation against reference charts."""
...
from jrs.domains.property.models import PropertyOutcomeTaxonomy
...
_FIXTURES_DIR = (
    ...
    / "fixtures" / "validation_charts" / "property_domain"
)


@pytest.fixture
def property_svc() -> PropertyDomainService:
    """Create a PropertyDomainService."""
...
def extract_evidence_records(
    chart: dict,
    property_svc: PropertyDomainService,
) -> tuple[EvidenceRecord, ...]:
    """Extract evidence records from chart natal facts using property rules."""
    facts = chart.get("natal_facts", {})
    return property_svc.evaluate_property_facts(facts)
...
def extract_event_windows(
    ...
    for d in dasha_raw:
        ...
        windows.append(EventWindow(
            candidate_event_taxonomy="PROPERTY_ACQUISITION",
            triggers=(trigger,),
        ))
...
def run_pipeline(
    ...
    """Run the full JRS Property pipeline for a single outcome.

    Steps:
        1. Extract Facts (natal_facts from chart)
        2. Generate Evidence Records (JRS-041 property rules)
    ...
    evidence_records = extract_evidence_records(chart, property_svc)
...
# ── Chart 1: Property Acquisition ────────────────────────────────────────────


class TestChart01PropertyAcquisition:
    """Validation tests for Chart 1 - property acquisition."""

    @pytest.fixture
    def chart(self) -> dict:
        return load_chart("chart_01_property_acquisition.json")

    def test_property_acquisition(
        self,
        chart: dict,
        property_svc: PropertyDomainService,
        convergence_svc: ConvergenceService,
        ...
        result = run_pipeline(
            chart, property_svc, convergence_svc, "PROPERTY_ACQUISITION",
        )
        expected = chart["expected_assessments"]["PROPERTY_ACQUISITION"]
        assert result["assessment_status"] == expected["assessment_status"]
        assert result["timing_status"] == expected["timing_status"]

    def test_real_estate_wealth(
        self,
        chart: dict,
        property_svc: PropertyDomainService,
        convergence_svc: ConvergenceService,
        ...
        result = run_pipeline(
            chart, property_svc, convergence_svc, "REAL_ESTATE_WEALTH",
        )
        expected = chart["expected_assessments"]["REAL_ESTATE_WEALTH"]
        assert result["assessment_status"] == expected["assessment_status"]
    ...
    def test_multiple_properties(
        ...
        result = run_pipeline(
            chart, property_svc, convergence_svc, "MULTIPLE_PROPERTIES",
        )
        expected = chart["expected_assessments"]["MULTIPLE_PROPERTIES"]
        assert result["assessment_status"] == expected["assessment_status"]


# ── Chart 2: Disputes Over Property ──────────────────────────────────────────


class TestChart02DisputesOverProperty:
    """Validation tests for Chart 2 - disputes over property."""

    @pytest.fixture
    def chart(self) -> dict:
        return load_chart("chart_02_disputes_over_property.json")

    def test_disputes_over_property(
        self,
        chart: dict,
        property_svc: PropertyDomainService,
        convergence_svc: ConvergenceService,
        ...
        result = run_pipeline(
            chart, property_svc, convergence_svc, "DISPUTES_OVER_PROPERTY",
        )
        expected = chart["expected_assessments"]["DISPUTES_OVER_PROPERTY"]
        assert result["assessment_status"] == expected["assessment_status"]
        assert result["timing_status"] == expected["timing_status"]
    ...
    def test_loss_of_property(
        ...
        result = run_pipeline(
            chart, property_svc, convergence_svc, "LOSS_OF_PROPERTY",
        )
        expected = chart["expected_assessments"]["LOSS_OF_PROPERTY"]
        assert result["assessment_status"] == expected["assessment_status"]


# ── Chart 3: Loss Of Property ────────────────────────────────────────────────


class TestChart03LossOfProperty:
    """Validation tests for Chart 3 - loss of property."""

    @pytest.fixture
    def chart(self) -> dict:
        return load_chart("chart_03_loss_of_property.json")
...
# ── Chart 4: Foreign Property ────────────────────────────────────────────────


class TestChart04ForeignProperty:
    """Validation tests for Chart 4 - foreign property."""

    @pytest.fixture
    def chart(self) -> dict:
        return load_chart("chart_04_foreign_property.json")

    def test_foreign_property(
        self,
        chart: dict,
        property_svc: PropertyDomainService,
        convergence_svc: ConvergenceService,
        ...
        result = run_pipeline(
            chart, property_svc, convergence_svc, "FOREIGN_PROPERTY",
        )
        expected = chart["expected_assessments"]["FOREIGN_PROPERTY"]
        assert result["assessment_status"] == expected["assessment_status"]
        assert result["timing_status"] == expected["timing_status"]
...
class TestCrossChartDeterminism:
    ...
    def test_all_charts_deterministic(
        ...
        chart_files = [
            "chart_01_property_acquisition.json",
            "chart_02_disputes_over_property.json",
            "chart_03_loss_of_property.json",
            "chart_04_foreign_property.json",
        ]
        ...
        for chart_file in chart_files:
            ...
            for outcome in chart["expected_assessments"]:
                r1 = run_pipeline(
                    chart, property_svc, convergence_svc, outcome,
                )
                r2 = run_pipeline(
                    chart, property_svc, convergence_svc, outcome,
                )
