"""Integration tests for Migration domain validation against reference charts."""
...
from jrs.domains.migration.models import MigrationOutcomeTaxonomy
...
_FIXTURES_DIR = (
    ...
    / "fixtures" / "validation_charts" / "migration_domain"
)


@pytest.fixture
def migration_svc() -> MigrationDomainService:
    """Create a MigrationDomainService."""
...
def extract_evidence_records(
    chart: dict,
    migration_svc: MigrationDomainService,
) -> tuple[EvidenceRecord, ...]:
    """Extract evidence records from chart natal facts using migration rules."""
    facts = chart.get("natal_facts", {})
    return migration_svc.evaluate_migration_facts(facts)
...
def extract_event_windows(
    ...
    for d in dasha_raw:
        ...
        windows.append(EventWindow(
            candidate_event_taxonomy="FOREIGN_SETTLEMENT",
            triggers=(trigger,),
        ))
...
def run_pipeline(
    ...
    """Run the full JRS Migration pipeline for a single outcome.

    Steps:
        1. Extract Facts (natal_facts from chart)
        2. Generate Evidence Records (JRS-037 migration rules)
    ...
    evidence_records = extract_evidence_records(chart, migration_svc)
...
# ── Chart 1: Foreign Settlement ──────────────────────────────────────────────


class TestChart01ForeignSettlement:
    """Validation tests for Chart 1 - foreign settlement."""

    @pytest.fixture
    def chart(self) -> dict:
        return load_chart("chart_01_foreign_settlement.json")

    def test_foreign_settlement(
        self,
        chart: dict,
        migration_svc: MigrationDomainService,
        convergence_svc: ConvergenceService,
        ...
        result = run_pipeline(
            chart, migration_svc, convergence_svc, "FOREIGN_SETTLEMENT",
        )
        expected = chart["expected_assessments"]["FOREIGN_SETTLEMENT"]
        assert result["assessment_status"] == expected["assessment_status"]
        assert result["timing_status"] == expected["timing_status"]

    def test_cross_cultural_success(
        self,
        chart: dict,
        migration_svc: MigrationDomainService,
        convergence_svc: ConvergenceService,
        ...
        result = run_pipeline(
            chart, migration_svc, convergence_svc, "CROSS_CULTURAL_SUCCESS",
        )
        expected = chart["expected_assessments"]["CROSS_CULTURAL_SUCCESS"]
        assert result["assessment_status"] == expected["assessment_status"]

    def test_short_term_travel(
        self,
        chart: dict,
        migration_svc: MigrationDomainService,
        convergence_svc: ConvergenceService,
        ...
        result = run_pipeline(
            chart, migration_svc, convergence_svc, "SHORT_TERM_TRAVEL",
        )
        expected = chart["expected_assessments"]["SHORT_TERM_TRAVEL"]
        assert result["assessment_status"] == expected["assessment_status"]


# ── Chart 2: Short Term Travel ───────────────────────────────────────────────


class TestChart02ShortTermTravel:
    """Validation tests for Chart 2 - short term travel."""

    @pytest.fixture
    def chart(self) -> dict:
        return load_chart("chart_02_short_term_travel.json")
...
# ── Chart 3: Migration Delay ─────────────────────────────────────────────────


class TestChart03MigrationDelay:
    """Validation tests for Chart 3 - migration delay."""

    @pytest.fixture
    def chart(self) -> dict:
        return load_chart("chart_03_migration_delay.json")

    def test_migration_delay(
        self,
        chart: dict,
        migration_svc: MigrationDomainService,
        convergence_svc: ConvergenceService,
        ...
        result = run_pipeline(
            chart, migration_svc, convergence_svc, "MIGRATION_DELAY",
        )
        expected = chart["expected_assessments"]["MIGRATION_DELAY"]
        assert result["assessment_status"] == expected["assessment_status"]
        assert result["timing_status"] == expected["timing_status"]
    ...
    def test_visa_obstacles(
        ...
        result = run_pipeline(
            chart, migration_svc, convergence_svc, "VISA_OBSTACLES",
        )
        expected = chart["expected_assessments"]["VISA_OBSTACLES"]
        assert result["assessment_status"] == expected["assessment_status"]


# ── Chart 4: Visa Obstacles ──────────────────────────────────────────────────


class TestChart04VisaObstacles:
    """Validation tests for Chart 4 - visa obstacles."""

    @pytest.fixture
    def chart(self) -> dict:
        return load_chart("chart_04_visa_obstacles.json")
...
class TestCrossChartDeterminism:
    ...
    def test_all_charts_deterministic(
        ...
        chart_files = [
            "chart_01_foreign_settlement.json",
            "chart_02_short_term_travel.json",
            "chart_03_migration_delay.json",
            "chart_04_visa_obstacles.json",
        ]
        ...
        for chart_file in chart_files:
            ...
            for outcome in chart["expected_assessments"]:
                r1 = run_pipeline(
                    chart, migration_svc, convergence_svc, outcome,
                )
                r2 = run_pipeline(
                    chart, migration_svc, convergence_svc, outcome,
                )
