"""Integration tests for Litigation domain validation against reference charts."""
...
from jrs.domains.litigation.models import LitigationOutcomeTaxonomy
...
_FIXTURES_DIR = (
    ...
    / "fixtures" / "validation_charts" / "litigation_domain"
)


@pytest.fixture
def litigation_svc() -> LitigationDomainService:
    """Create a LitigationDomainService."""
...
def extract_evidence_records(
    chart: dict,
    litigation_svc: LitigationDomainService,
) -> tuple[EvidenceRecord, ...]:
    """Extract evidence records from chart natal facts using litigation rules."""
    facts = chart.get("natal_facts", {})
    return litigation_svc.evaluate_litigation_facts(facts)
...
def extract_event_windows(
    ...
    for d in dasha_raw:
        ...
        windows.append(EventWindow(
            candidate_event_taxonomy="LEGAL_VICTORY",
            triggers=(trigger,),
        ))
...
def run_pipeline(
    ...
    """Run the full JRS Litigation pipeline for a single outcome.

    Steps:
        1. Extract Facts (natal_facts from chart)
        2. Generate Evidence Records (JRS-027 litigation rules)
    ...
    evidence_records = extract_evidence_records(chart, litigation_svc)
...
# ── Chart 1: Legal Victory ───────────────────────────────────────────────────


class TestChart01LegalVictory:
    """Validation tests for Chart 1 - legal victory."""

    @pytest.fixture
    def chart(self) -> dict:
        return load_chart("chart_01_legal_victory.json")

    def test_legal_victory(
        self,
        chart: dict,
        litigation_svc: LitigationDomainService,
        convergence_svc: ConvergenceService,
        ...
        result = run_pipeline(
            chart, litigation_svc, convergence_svc, "LEGAL_VICTORY",
        )
        expected = chart["expected_assessments"]["LEGAL_VICTORY"]
        assert result["assessment_status"] == expected["assessment_status"]
        assert result["timing_status"] == expected["timing_status"]

    def test_prolonged_litigation(
        self,
        chart: dict,
        litigation_svc: LitigationDomainService,
        convergence_svc: ConvergenceService,
        ...
        result = run_pipeline(
            chart, litigation_svc, convergence_svc, "PROLONGED_LITIGATION",
        )
        expected = chart["expected_assessments"]["PROLONGED_LITIGATION"]
        assert result["assessment_status"] == expected["assessment_status"]
    ...
    def test_settlement(
        ...
        result = run_pipeline(
            chart, litigation_svc, convergence_svc, "SETTLEMENT_OUT_OF_COURT",
        )
        expected = chart["expected_assessments"]["SETTLEMENT_OUT_OF_COURT"]
        assert result["assessment_status"] == expected["assessment_status"]


# ── Chart 2: Prolonged Litigation ────────────────────────────────────────────


class TestChart02ProlongedLitigation:
    """Validation tests for Chart 2 - prolonged litigation."""

    @pytest.fixture
    def chart(self) -> dict:
        return load_chart("chart_02_prolonged_litigation.json")
...
# ── Chart 3: Settlement Out of Court ─────────────────────────────────────────


class TestChart03Settlement:
    """Validation tests for Chart 3 - settlement out of court."""

    @pytest.fixture
    def chart(self) -> dict:
        return load_chart("chart_03_settlement.json")
...
# ── Chart 4: False Accusation ────────────────────────────────────────────────


class TestChart04FalseAccusation:
    """Validation tests for Chart 4 - false accusation."""

    @pytest.fixture
    def chart(self) -> dict:
        return load_chart("chart_04_false_accusation.json")
    ...
    def test_false_accusation(
        ...
        result = run_pipeline(
            chart, litigation_svc, convergence_svc, "FALSE_ACCUSATION",
        )
        expected = chart["expected_assessments"]["FALSE_ACCUSATION"]
        assert result["assessment_status"] == expected["assessment_status"]
        assert result["timing_status"] == expected["timing_status"]
...
class TestCrossChartDeterminism:
    ...
    def test_all_charts_deterministic(
        ...
        chart_files = [
            "chart_01_legal_victory.json",
            "chart_02_prolonged_litigation.json",
            "chart_03_settlement.json",
            "chart_04_false_accusation.json",
        ]
        ...
        for chart_file in chart_files:
            ...
            for outcome in chart["expected_assessments"]:
                r1 = run_pipeline(
                    chart, litigation_svc, convergence_svc, outcome,
                )
                r2 = run_pipeline(
                    chart, litigation_svc, convergence_svc, outcome,
                )
