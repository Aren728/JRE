"""Integration tests for Health domain validation against reference charts.

SAFETY CONSTRAINT: This test module uses ONLY traditional vitality and
constitution terminology. No medical, diagnostic, or mortality language
is used in fixtures, assertions, or test descriptions.
"""

from __future__ import annotations

import json
from pathlib import Path

import pytest

from jrs.convergence.models import AssessmentStatus, TimingStatus, OverallEvidenceStrength
from jrs.convergence.service import ConvergenceService
from jrs.domains.health.models import HealthOutcomeTaxonomy
from jrs.domains.health.service import HealthDomainService
from jrs.evidence.models import EvidenceRecord
from jrs.temporal.models import ActivationType, EventWindow, TemporalTrigger

_FIXTURES_DIR = (
    Path(__file__).resolve().parent.parent.parent.parent
    / "fixtures" / "validation_charts" / "health_domain"
)


@pytest.fixture
def health_svc() -> HealthDomainService:
    """Create a HealthDomainService."""
    return HealthDomainService()


@pytest.fixture
def convergence_svc() -> ConvergenceService:
    """Create a ConvergenceService."""
    return ConvergenceService()


def load_chart(chart_file: str) -> dict:
    """Load a reference chart from the fixtures directory."""
    path = _FIXTURES_DIR / chart_file
    with path.open() as f:
        return json.load(f)


def extract_evidence_records(
    chart: dict,
    health_svc: HealthDomainService,
) -> tuple[EvidenceRecord, ...]:
    """Extract evidence records from chart natal facts using health rules."""
    facts = chart.get("natal_facts", {})
    return health_svc.evaluate_health_facts(facts)


def extract_event_windows(
    chart: dict,
) -> tuple[EventWindow, ...]:
    """Extract event windows from chart dasha and transit periods."""
    windows: list[EventWindow] = []

    dasha_raw = chart.get("dasha_periods", [])
    for d in dasha_raw:
        trigger = TemporalTrigger(
            activation_type=ActivationType(d["activation_type"]),
            triggering_planet=d.get("triggering_planet", ""),
            activation_start_utc=d.get("activation_start_utc", ""),
            activation_end_utc=d.get("activation_end_utc", ""),
            strength=float(d.get("strength", 1.0)),
        )
        windows.append(EventWindow(
            candidate_event_taxonomy="HIGH_VITALITY",
            triggers=(trigger,),
        ))

    transit_raw = chart.get("transits", [])
    for t in transit_raw:
        trigger = TemporalTrigger(
            activation_type=ActivationType(t["activation_type"]),
            triggering_planet=t.get("triggering_planet", ""),
            activation_start_utc=t.get("activation_start_utc", ""),
            activation_end_utc=t.get("activation_end_utc", ""),
            strength=float(t.get("strength", 1.0)),
        )
        windows.append(EventWindow(
            candidate_event_taxonomy="HIGH_VITALITY",
            triggers=(trigger,),
        ))

    return tuple(windows)


def run_pipeline(
    chart: dict,
    health_svc: HealthDomainService,
    convergence_svc: ConvergenceService,
    outcome_taxonomy: str,
) -> dict:
    """Run the full JRS Health pipeline for a single outcome.

    Steps:
        1. Extract Facts (natal_facts from chart)
        2. Generate Evidence Records (JRS-027 health rules)
        3. Calculate Temporal Windows (JRS-028 dasha/transits)
        4. Assess Convergence (JRS-029)
    """
    # Step 1 & 2: Extract facts and generate evidence records
    evidence_records = extract_evidence_records(chart, health_svc)

    # Step 3: Calculate temporal windows
    event_windows = extract_event_windows(chart)

    # Step 4: Assess convergence
    assessment = convergence_svc.assess_domain(
        outcome_taxonomy,
        evidence_records=evidence_records,
        event_windows=event_windows,
    )

    return assessment.to_dict()


# ── Chart 1: High Vitality ───────────────────────────────────────────────────


class TestChart01HighVitality:
    """Validation tests for Chart 1 - high constitutional vitality."""

    @pytest.fixture
    def chart(self) -> dict:
        return load_chart("chart_01_high_vitality.json")

    def test_high_vitality(
        self,
        chart: dict,
        health_svc: HealthDomainService,
        convergence_svc: ConvergenceService,
    ) -> None:
        result = run_pipeline(
            chart, health_svc, convergence_svc, "HIGH_VITALITY",
        )
        expected = chart["expected_assessments"]["HIGH_VITALITY"]
        assert result["assessment_status"] == expected["assessment_status"]
        assert result["timing_status"] == expected["timing_status"]

    def test_low_vitality(
        self,
        chart: dict,
        health_svc: HealthDomainService,
        convergence_svc: ConvergenceService,
    ) -> None:
        result = run_pipeline(
            chart, health_svc, convergence_svc, "LOW_VITALITY",
        )
        expected = chart["expected_assessments"]["LOW_VITALITY"]
        assert result["assessment_status"] == expected["assessment_status"]

    def test_strong_recovery(
        self,
        chart: dict,
        health_svc: HealthDomainService,
        convergence_svc: ConvergenceService,
    ) -> None:
        result = run_pipeline(
            chart, health_svc, convergence_svc, "STRONG_RECOVERY_CAPACITY",
        )
        expected = chart["expected_assessments"]["STRONG_RECOVERY_CAPACITY"]
        assert result["assessment_status"] == expected["assessment_status"]


# ── Chart 2: Chronic Stress ──────────────────────────────────────────────────


class TestChart02ChronicStress:
    """Validation tests for Chart 2 - chronic constitutional strain."""

    @pytest.fixture
    def chart(self) -> dict:
        return load_chart("chart_02_chronic_stress.json")

    def test_chronic_stress(
        self,
        chart: dict,
        health_svc: HealthDomainService,
        convergence_svc: ConvergenceService,
    ) -> None:
        result = run_pipeline(
            chart, health_svc, convergence_svc, "CHRONIC_STRESS",
        )
        expected = chart["expected_assessments"]["CHRONIC_STRESS"]
        assert result["assessment_status"] == expected["assessment_status"]
        assert result["timing_status"] == expected["timing_status"]

    def test_high_vitality(
        self,
        chart: dict,
        health_svc: HealthDomainService,
        convergence_svc: ConvergenceService,
    ) -> None:
        result = run_pipeline(
            chart, health_svc, convergence_svc, "HIGH_VITALITY",
        )
        expected = chart["expected_assessments"]["HIGH_VITALITY"]
        assert result["assessment_status"] == expected["assessment_status"]

    def test_energy_fluctuations(
        self,
        chart: dict,
        health_svc: HealthDomainService,
        convergence_svc: ConvergenceService,
    ) -> None:
        result = run_pipeline(
            chart, health_svc, convergence_svc, "ENERGY_FLUCTUATIONS",
        )
        expected = chart["expected_assessments"]["ENERGY_FLUCTUATIONS"]
        assert result["assessment_status"] == expected["assessment_status"]


# ── Chart 3: Strong Recovery Capacity ────────────────────────────────────────


class TestChart03StrongRecovery:
    """Validation tests for Chart 3 - strong recuperative capacity."""

    @pytest.fixture
    def chart(self) -> dict:
        return load_chart("chart_03_strong_recovery.json")

    def test_strong_recovery(
        self,
        chart: dict,
        health_svc: HealthDomainService,
        convergence_svc: ConvergenceService,
    ) -> None:
        result = run_pipeline(
            chart, health_svc, convergence_svc, "STRONG_RECOVERY_CAPACITY",
        )
        expected = chart["expected_assessments"]["STRONG_RECOVERY_CAPACITY"]
        assert result["assessment_status"] == expected["assessment_status"]
        assert result["timing_status"] == expected["timing_status"]

    def test_high_vitality(
        self,
        chart: dict,
        health_svc: HealthDomainService,
        convergence_svc: ConvergenceService,
    ) -> None:
        result = run_pipeline(
            chart, health_svc, convergence_svc, "HIGH_VITALITY",
        )
        expected = chart["expected_assessments"]["HIGH_VITALITY"]
        assert result["assessment_status"] == expected["assessment_status"]

    def test_chronic_stress(
        self,
        chart: dict,
        health_svc: HealthDomainService,
        convergence_svc: ConvergenceService,
    ) -> None:
        result = run_pipeline(
            chart, health_svc, convergence_svc, "CHRONIC_STRESS",
        )
        expected = chart["expected_assessments"]["CHRONIC_STRESS"]
        assert result["assessment_status"] == expected["assessment_status"]


# ── Chart 4: Energy Fluctuations ─────────────────────────────────────────────


class TestChart04EnergyFluctuations:
    """Validation tests for Chart 4 - variable constitutional energy."""

    @pytest.fixture
    def chart(self) -> dict:
        return load_chart("chart_04_energy_fluctuations.json")

    def test_energy_fluctuations(
        self,
        chart: dict,
        health_svc: HealthDomainService,
        convergence_svc: ConvergenceService,
    ) -> None:
        result = run_pipeline(
            chart, health_svc, convergence_svc, "ENERGY_FLUCTUATIONS",
        )
        expected = chart["expected_assessments"]["ENERGY_FLUCTUATIONS"]
        assert result["assessment_status"] == expected["assessment_status"]
        assert result["timing_status"] == expected["timing_status"]

    def test_high_vitality(
        self,
        chart: dict,
        health_svc: HealthDomainService,
        convergence_svc: ConvergenceService,
    ) -> None:
        result = run_pipeline(
            chart, health_svc, convergence_svc, "HIGH_VITALITY",
        )
        expected = chart["expected_assessments"]["HIGH_VITALITY"]
        assert result["assessment_status"] == expected["assessment_status"]

    def test_chronic_stress(
        self,
        chart: dict,
        health_svc: HealthDomainService,
        convergence_svc: ConvergenceService,
    ) -> None:
        result = run_pipeline(
            chart, health_svc, convergence_svc, "CHRONIC_STRESS",
        )
        expected = chart["expected_assessments"]["CHRONIC_STRESS"]
        assert result["assessment_status"] == expected["assessment_status"]


# ── Cross-Chart Determinism ──────────────────────────────────────────────────


class TestCrossChartDeterminism:
    """Test that the pipeline produces deterministic results."""

    def test_all_charts_deterministic(
        self,
        health_svc: HealthDomainService,
        convergence_svc: ConvergenceService,
    ) -> None:
        """Verify byte-identical output for identical inputs across all charts."""
        chart_files = [
            "chart_01_high_vitality.json",
            "chart_02_chronic_stress.json",
            "chart_03_strong_recovery.json",
            "chart_04_energy_fluctuations.json",
        ]

        for chart_file in chart_files:
            chart = load_chart(chart_file)
            for outcome in chart["expected_assessments"]:
                r1 = run_pipeline(
                    chart, health_svc, convergence_svc, outcome,
                )
                r2 = run_pipeline(
                    chart, health_svc, convergence_svc, outcome,
                )
                assert json.dumps(r1, sort_keys=True) == json.dumps(
                    r2, sort_keys=True,
                ), f"Non-deterministic output for {chart_file}/{outcome}"


# ── Fixture Validation ───────────────────────────────────────────────────────


class TestFixturesValid:
    """Validate that all fixture files are well-formed."""

    def test_all_fixtures_exist(self) -> None:
        expected_files = [
            "chart_01_high_vitality.json",
            "chart_02_chronic_stress.json",
            "chart_03_strong_recovery.json",
            "chart_04_energy_fluctuations.json",
        ]
        for f in expected_files:
            assert (_FIXTURES_DIR / f).exists(), f"Missing fixture: {f}"

    def test_all_fixtures_have_required_fields(self) -> None:
        chart_files = [
            "chart_01_high_vitality.json",
            "chart_02_chronic_stress.json",
            "chart_03_strong_recovery.json",
            "chart_04_energy_fluctuations.json",
        ]
        for chart_file in chart_files:
            chart = load_chart(chart_file)
            assert "chart_id" in chart, f"{chart_file} missing chart_id"
            assert "natal_facts" in chart, f"{chart_file} missing natal_facts"
            assert "expected_assessments" in chart, f"{chart_file} missing expected_assessments"
            assert len(chart["expected_assessments"]) >= 3, (
                f"{chart_file} should have at least 3 expected assessments"
            )

    def test_distinct_outcomes_across_charts(self) -> None:
        """Verify at least 4 distinct vitality outcome types are tested."""
        all_outcomes: set[str] = set()
        chart_files = [
            "chart_01_high_vitality.json",
            "chart_02_chronic_stress.json",
            "chart_03_strong_recovery.json",
            "chart_04_energy_fluctuations.json",
        ]
        for chart_file in chart_files:
            chart = load_chart(chart_file)
            all_outcomes.update(chart["expected_assessments"].keys())
        assert len(all_outcomes) >= 4, (
            f"Expected at least 4 distinct outcomes, got {len(all_outcomes)}: {all_outcomes}"
        )

    def test_no_medical_terminology_in_fixtures(self) -> None:
        """CRITICAL SAFETY: Verify no medical terms in fixture descriptions."""
        medical_terms = {
            "disease", "death", "surgery", "diagnosis", "medical",
            "illness", "sickness", "cancer", "tumor", "infection",
        }
        chart_files = [
            "chart_01_high_vitality.json",
            "chart_02_chronic_stress.json",
            "chart_03_strong_recovery.json",
            "chart_04_energy_fluctuations.json",
        ]
        for chart_file in chart_files:
            chart = load_chart(chart_file)
            desc = chart.get("description", "").lower()
            for event in chart.get("known_events", []):
                desc += " " + event.get("description", "").lower()
            for term in medical_terms:
                assert term not in desc, (
                    f"{chart_file} contains forbidden medical term '{term}'"
                )
