"""Integration tests for Marriage domain validation against reference charts."""

from __future__ import annotations

import json
from pathlib import Path

import pytest

from jrs.convergence.models import AssessmentStatus, TimingStatus, OverallEvidenceStrength
from jrs.convergence.service import ConvergenceService
from jrs.domains.marriage.models import MarriageOutcomeTaxonomy
from jrs.domains.marriage.service import MarriageDomainService
from jrs.evidence.models import EvidenceRecord
from jrs.temporal.models import ActivationType, EventWindow, TemporalTrigger

_FIXTURES_DIR = (
    Path(__file__).resolve().parent.parent.parent.parent
    / "fixtures" / "validation_charts" / "marriage_domain"
)


@pytest.fixture
def marriage_svc() -> MarriageDomainService:
    """Create a MarriageDomainService."""
    return MarriageDomainService()


@pytest.fixture
def convergence_svc() -> ConvergenceService:
    """Create a ConvergenceService."""
    return ConvergenceService()


def load_chart(chart_file: str) -> dict:
    """Load a reference chart from the fixtures directory."""
    path = _FIXTURES_DIR / chart_file
    with path.open() as f:
        return json.load(f)


def extract_evidence_records(
    chart: dict,
    marriage_svc: MarriageDomainService,
) -> tuple[EvidenceRecord, ...]:
    """Extract evidence records from chart natal facts using marriage rules."""
    facts = chart.get("natal_facts", {})
    return marriage_svc.evaluate_marriage_facts(facts)


def extract_event_windows(
    chart: dict,
) -> tuple[EventWindow, ...]:
    """Extract event windows from chart dasha and transit periods."""
    windows: list[EventWindow] = []

    dasha_raw = chart.get("dasha_periods", [])
    for d in dasha_raw:
        trigger = TemporalTrigger(
            activation_type=ActivationType(d["activation_type"]),
            triggering_planet=d.get("triggering_planet", ""),
            activation_start_utc=d.get("activation_start_utc", ""),
            activation_end_utc=d.get("activation_end_utc", ""),
            strength=float(d.get("strength", 1.0)),
        )
        windows.append(EventWindow(
            candidate_event_taxonomy="MARRIAGE_FORMATION",
            triggers=(trigger,),
        ))

    transit_raw = chart.get("transits", [])
    for t in transit_raw:
        trigger = TemporalTrigger(
            activation_type=ActivationType(t["activation_type"]),
            triggering_planet=t.get("triggering_planet", ""),
            activation_start_utc=t.get("activation_start_utc", ""),
            activation_end_utc=t.get("activation_end_utc", ""),
            strength=float(t.get("strength", 1.0)),
        )
        windows.append(EventWindow(
            candidate_event_taxonomy="MARRIAGE_FORMATION",
            triggers=(trigger,),
        ))

    return tuple(windows)


def run_pipeline(
    chart: dict,
    marriage_svc: MarriageDomainService,
    convergence_svc: ConvergenceService,
    outcome_taxonomy: str,
) -> dict:
    """Run the full JRS Marriage pipeline for a single outcome.

    Steps:
        1. Extract Facts (natal_facts from chart)
        2. Generate Evidence Records (JRS-027 marriage rules)
        3. Calculate Temporal Windows (JRS-028 dasha/transits)
        4. Assess Convergence (JRS-029)
    """
    # Step 1 & 2: Extract facts and generate evidence records
    evidence_records = extract_evidence_records(chart, marriage_svc)

    # Step 3: Calculate temporal windows
    event_windows = extract_event_windows(chart)

    # Step 4: Assess convergence
    assessment = convergence_svc.assess_domain(
        outcome_taxonomy,
        evidence_records=evidence_records,
        event_windows=event_windows,
    )

    return assessment.to_dict()


# ── Chart 1: Harmonious Marriage ─────────────────────────────────────────────


class TestChart01HarmoniousMarriage:
    """Validation tests for Chart 1 - harmonious marriage."""

    @pytest.fixture
    def chart(self) -> dict:
        return load_chart("chart_01_harmonious_marriage.json")

    def test_marriage_formation(
        self,
        chart: dict,
        marriage_svc: MarriageDomainService,
        convergence_svc: ConvergenceService,
    ) -> None:
        result = run_pipeline(
            chart, marriage_svc, convergence_svc, "MARRIAGE_FORMATION",
        )
        expected = chart["expected_assessments"]["MARRIAGE_FORMATION"]
        assert result["assessment_status"] == expected["assessment_status"]
        assert result["timing_status"] == expected["timing_status"]

    def test_marital_harmony(
        self,
        chart: dict,
        marriage_svc: MarriageDomainService,
        convergence_svc: ConvergenceService,
    ) -> None:
        result = run_pipeline(
            chart, marriage_svc, convergence_svc, "MARITAL_HARMONY",
        )
        expected = chart["expected_assessments"]["MARITAL_HARMONY"]
        assert result["assessment_status"] == expected["assessment_status"]

    def test_delayed_marriage_contradicted(
        self,
        chart: dict,
        marriage_svc: MarriageDomainService,
        convergence_svc: ConvergenceService,
    ) -> None:
        result = run_pipeline(
            chart, marriage_svc, convergence_svc, "DELAYED_MARRIAGE",
        )
        expected = chart["expected_assessments"]["DELAYED_MARRIAGE"]
        assert result["assessment_status"] == expected["assessment_status"]

    def test_separation_contradicted(
        self,
        chart: dict,
        marriage_svc: MarriageDomainService,
        convergence_svc: ConvergenceService,
    ) -> None:
        result = run_pipeline(
            chart, marriage_svc, convergence_svc, "SEPARATION",
        )
        expected = chart["expected_assessments"]["SEPARATION"]
        assert result["assessment_status"] == expected["assessment_status"]


# ── Chart 2: Delayed Marriage ────────────────────────────────────────────────


class TestChart02DelayedMarriage:
    """Validation tests for Chart 2 - delayed marriage."""

    @pytest.fixture
    def chart(self) -> dict:
        return load_chart("chart_02_delayed_marriage.json")

    def test_delayed_marriage_supported(
        self,
        chart: dict,
        marriage_svc: MarriageDomainService,
        convergence_svc: ConvergenceService,
    ) -> None:
        result = run_pipeline(
            chart, marriage_svc, convergence_svc, "DELAYED_MARRIAGE",
        )
        expected = chart["expected_assessments"]["DELAYED_MARRIAGE"]
        assert result["assessment_status"] == expected["assessment_status"]
        assert result["timing_status"] == expected["timing_status"]

    def test_late_marriage_supported(
        self,
        chart: dict,
        marriage_svc: MarriageDomainService,
        convergence_svc: ConvergenceService,
    ) -> None:
        result = run_pipeline(
            chart, marriage_svc, convergence_svc, "LATE_MARRIAGE",
        )
        expected = chart["expected_assessments"]["LATE_MARRIAGE"]
        assert result["assessment_status"] == expected["assessment_status"]

    def test_marriage_formation_weak(
        self,
        chart: dict,
        marriage_svc: MarriageDomainService,
        convergence_svc: ConvergenceService,
    ) -> None:
        result = run_pipeline(
            chart, marriage_svc, convergence_svc, "MARRIAGE_FORMATION",
        )
        expected = chart["expected_assessments"]["MARRIAGE_FORMATION"]
        assert result["assessment_status"] == expected["assessment_status"]


# ── Chart 3: Separation ──────────────────────────────────────────────────────


class TestChart03Separation:
    """Validation tests for Chart 3 - separation."""

    @pytest.fixture
    def chart(self) -> dict:
        return load_chart("chart_03_separation.json")

    def test_separation_supported(
        self,
        chart: dict,
        marriage_svc: MarriageDomainService,
        convergence_svc: ConvergenceService,
    ) -> None:
        result = run_pipeline(
            chart, marriage_svc, convergence_svc, "SEPARATION",
        )
        expected = chart["expected_assessments"]["SEPARATION"]
        assert result["assessment_status"] == expected["assessment_status"]
        assert result["timing_status"] == expected["timing_status"]

    def test_marital_conflict_supported(
        self,
        chart: dict,
        marriage_svc: MarriageDomainService,
        convergence_svc: ConvergenceService,
    ) -> None:
        result = run_pipeline(
            chart, marriage_svc, convergence_svc, "MARITAL_CONFLICT",
        )
        expected = chart["expected_assessments"]["MARITAL_CONFLICT"]
        assert result["assessment_status"] == expected["assessment_status"]

    def test_marital_harmony_contradicted(
        self,
        chart: dict,
        marriage_svc: MarriageDomainService,
        convergence_svc: ConvergenceService,
    ) -> None:
        result = run_pipeline(
            chart, marriage_svc, convergence_svc, "MARITAL_HARMONY",
        )
        expected = chart["expected_assessments"]["MARITAL_HARMONY"]
        assert result["assessment_status"] == expected["assessment_status"]


# ── Chart 4: Love Marriage ───────────────────────────────────────────────────


class TestChart04LoveMarriage:
    """Validation tests for Chart 4 - love marriage."""

    @pytest.fixture
    def chart(self) -> dict:
        return load_chart("chart_04_love_marriage.json")

    def test_love_marriage_supported(
        self,
        chart: dict,
        marriage_svc: MarriageDomainService,
        convergence_svc: ConvergenceService,
    ) -> None:
        result = run_pipeline(
            chart, marriage_svc, convergence_svc, "LOVE_MARRIAGE",
        )
        expected = chart["expected_assessments"]["LOVE_MARRIAGE"]
        assert result["assessment_status"] == expected["assessment_status"]
        assert result["timing_status"] == expected["timing_status"]

    def test_marriage_formation_strong(
        self,
        chart: dict,
        marriage_svc: MarriageDomainService,
        convergence_svc: ConvergenceService,
    ) -> None:
        result = run_pipeline(
            chart, marriage_svc, convergence_svc, "MARRIAGE_FORMATION",
        )
        expected = chart["expected_assessments"]["MARRIAGE_FORMATION"]
        assert result["assessment_status"] == expected["assessment_status"]

    def test_arranged_marriage_contradicted(
        self,
        chart: dict,
        marriage_svc: MarriageDomainService,
        convergence_svc: ConvergenceService,
    ) -> None:
        result = run_pipeline(
            chart, marriage_svc, convergence_svc, "ARRANGED_MARRIAGE",
        )
        expected = chart["expected_assessments"]["ARRANGED_MARRIAGE"]
        assert result["assessment_status"] == expected["assessment_status"]


# ── Chart 5: Remarriage After Divorce ────────────────────────────────────────


class TestChart05Remarriage:
    """Validation tests for Chart 5 - remarriage after divorce."""

    @pytest.fixture
    def chart(self) -> dict:
        return load_chart("chart_05_remarriage.json")

    def test_remarrige_divorce_supported(
        self,
        chart: dict,
        marriage_svc: MarriageDomainService,
        convergence_svc: ConvergenceService,
    ) -> None:
        result = run_pipeline(
            chart, marriage_svc, convergence_svc, "REMARRIAGE_AFTER_DIVORCE",
        )
        expected = chart["expected_assessments"]["REMARRIAGE_AFTER_DIVORCE"]
        assert result["assessment_status"] == expected["assessment_status"]
        assert result["timing_status"] == expected["timing_status"]

    def test_separation_supported(
        self,
        chart: dict,
        marriage_svc: MarriageDomainService,
        convergence_svc: ConvergenceService,
    ) -> None:
        result = run_pipeline(
            chart, marriage_svc, convergence_svc, "SEPARATION",
        )
        expected = chart["expected_assessments"]["SEPARATION"]
        assert result["assessment_status"] == expected["assessment_status"]

    def test_marriage_formation_supported(
        self,
        chart: dict,
        marriage_svc: MarriageDomainService,
        convergence_svc: ConvergenceService,
    ) -> None:
        result = run_pipeline(
            chart, marriage_svc, convergence_svc, "MARRIAGE_FORMATION",
        )
        expected = chart["expected_assessments"]["MARRIAGE_FORMATION"]
        assert result["assessment_status"] == expected["assessment_status"]


# ── Cross-Chart Determinism ──────────────────────────────────────────────────


class TestCrossChartDeterminism:
    """Test that the pipeline produces deterministic results."""

    def test_all_charts_deterministic(
        self,
        marriage_svc: MarriageDomainService,
        convergence_svc: ConvergenceService,
    ) -> None:
        """Verify byte-identical output for identical inputs across all charts."""
        chart_files = [
            "chart_01_harmonious_marriage.json",
            "chart_02_delayed_marriage.json",
            "chart_03_separation.json",
            "chart_04_love_marriage.json",
            "chart_05_remarriage.json",
        ]

        for chart_file in chart_files:
            chart = load_chart(chart_file)
            for outcome in chart["expected_assessments"]:
                r1 = run_pipeline(
                    chart, marriage_svc, convergence_svc, outcome,
                )
                r2 = run_pipeline(
                    chart, marriage_svc, convergence_svc, outcome,
                )
                assert json.dumps(r1, sort_keys=True) == json.dumps(
                    r2, sort_keys=True,
                ), f"Non-deterministic output for {chart_file}/{outcome}"


# ── Fixture Validation ───────────────────────────────────────────────────────


class TestFixturesValid:
    """Validate that all fixture files are well-formed."""

    def test_all_fixtures_exist(self) -> None:
        expected_files = [
            "chart_01_harmonious_marriage.json",
            "chart_02_delayed_marriage.json",
            "chart_03_separation.json",
            "chart_04_love_marriage.json",
            "chart_05_remarriage.json",
        ]
        for f in expected_files:
            assert (_FIXTURES_DIR / f).exists(), f"Missing fixture: {f}"

    def test_all_fixtures_have_required_fields(self) -> None:
        chart_files = [
            "chart_01_harmonious_marriage.json",
            "chart_02_delayed_marriage.json",
            "chart_03_separation.json",
            "chart_04_love_marriage.json",
            "chart_05_remarriage.json",
        ]
        for chart_file in chart_files:
            chart = load_chart(chart_file)
            assert "chart_id" in chart, f"{chart_file} missing chart_id"
            assert "natal_facts" in chart, f"{chart_file} missing natal_facts"
            assert "expected_assessments" in chart, f"{chart_file} missing expected_assessments"
            assert len(chart["expected_assessments"]) >= 3, (
                f"{chart_file} should have at least 3 expected assessments"
            )

    def test_distinct_outcomes_across_charts(self) -> None:
        """Verify at least 3 distinct outcome types are tested."""
        all_outcomes: set[str] = set()
        chart_files = [
            "chart_01_harmonious_marriage.json",
            "chart_02_delayed_marriage.json",
            "chart_03_separation.json",
            "chart_04_love_marriage.json",
            "chart_05_remarriage.json",
        ]
        for chart_file in chart_files:
            chart = load_chart(chart_file)
            all_outcomes.update(chart["expected_assessments"].keys())
        assert len(all_outcomes) >= 3, (
            f"Expected at least 3 distinct outcomes, got {len(all_outcomes)}: {all_outcomes}"
        )
