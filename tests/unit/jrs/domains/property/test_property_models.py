"""Unit tests for property domain models and fact evaluation logic."""
...
import pytest
from tests.unit.jrs.domains.property.conftest import make_property_rule
...
class TestPropertyOutcomeTaxonomy:
    """Tests for the PropertyOutcomeTaxonomy enum."""

    def test_all_outcomes_have_string_values(self) -> None:
        for o in PropertyOutcomeTaxonomy:
            assert isinstance(o.value, str)
            assert o.value == o.name

    def test_outcome_count(self) -> None:
        assert len(PropertyOutcomeTaxonomy) == 6

    def test_outcome_from_value(self) -> None:
        acq = PropertyOutcomeTaxonomy("PROPERTY_ACQUISITION")
        assert acq is PropertyOutcomeTaxonomy.PROPERTY_ACQUISITION
        disp = PropertyOutcomeTaxonomy("DISPUTES_OVER_PROPERTY")
        assert disp is PropertyOutcomeTaxonomy.DISPUTES_OVER_PROPERTY

    def test_invalid_outcome(self) -> None:
        with pytest.raises(ValueError):
            PropertyOutcomeTaxonomy("INVALID")


class TestPropertyRule:
    """Tests for the PropertyRule model."""

    def test_creation(self) -> None:
        rule = make_property_rule(rule_id="R-TEST")
        assert rule.rule_id == "R-TEST"
        assert rule.outcome is PropertyOutcomeTaxonomy.PROPERTY_ACQUISITION

    def test_frozen(self) -> None:
        rule = make_property_rule()
    ...
    def test_to_dict(self) -> None:
        ...
        assert d["outcome"] == "LOSS_OF_PROPERTY"
...
class TestPropertyRuleCatalog:
    """Tests for the PropertyRuleCatalog model."""

    def test_creation(self) -> None:
        catalog = PropertyRuleCatalog(rules=(make_property_rule(),))
        assert len(catalog.rules) == 1

    def test_get_rules_by_outcome(self, sample_catalog: PropertyRuleCatalog) -> None:
        acq = sample_catalog.get_rules_by_outcome(PropertyOutcomeTaxonomy.PROPERTY_ACQUISITION)
        assert len(acq) == 1

    def test_get_rule_by_id(self, sample_catalog: PropertyRuleCatalog) -> None:
        rule = sample_catalog.get_rule_by_id("R-TEST-002")
        assert rule is not None
        assert rule.rule_id == "R-TEST-002"

    def test_get_rule_by_id_not_found(self, sample_catalog: PropertyRuleCatalog) -> None:
        rule = sample_catalog.get_rule_by_id("R-NONE")
        assert rule is None

    def test_to_dict(self, sample_catalog: PropertyRuleCatalog) -> None:
...
class TestEvaluateRule:
    ...
    def test_single_condition_match(self) -> None:
        rule = make_property_rule(condition_facts=("flag_a=true",))
        record = evaluate_rule(rule, {"flag_a": True})
        assert record is not None
        assert isinstance(record, EvidenceRecord)
        assert record.outcome_taxonomy == PropertyOutcomeTaxonomy.PROPERTY_ACQUISITION.value
    ...
    def test_multiple_conditions_all_match(self) -> None:
        rule = make_property_rule(condition_facts=("a=true", "b=true"))
    ...
    def test_empty_conditions_returns_none(self) -> None:
        rule = make_property_rule(condition_facts=())
        record = evaluate_rule(rule, {"flag_a": True})
        assert record is None
...
class TestEvaluateFacts:
    ...
    def test_multiple_rules(self, sample_rules: tuple[PropertyRule, ...]) -> None:
        facts = {
            "4th_lord_strong": True,
            "mars_afflicting_4th": True,
        }
        records = evaluate_facts(sample_rules, facts)
        assert len(records) >= 2

    def test_no_rules_match(self, sample_rules: tuple[PropertyRule, ...]) -> None:
        records = evaluate_facts(sample_rules, {})
        assert records == ()

    def test_deterministic_output(self, sample_rules: tuple[PropertyRule, ...]) -> None:
    ...
    def test_no_false_cross_contamination(self) -> None:
        """Ensure 'property acquisition' rule doesn't trigger 'property disputes'."""
        rules = (
            make_property_rule(
                rule_id="R-ACQ",
        ...
        assert records[0].outcome_taxonomy == PropertyOutcomeTaxonomy.PROPERTY_ACQUISITION.value

    def test_numeric_comparison(self) -> None:
        rules = (
            make_property_rule(
                rule_id="R-BALA",
                outcome=PropertyOutcomeTaxonomy.MULTIPLE_PROPERTIES,
...
class TestPropertyConfig:
    """Tests for the PropertyConfig model."""
