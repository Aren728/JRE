"""Unit tests for career domain models and fact evaluation logic."""
...
from tests.unit.jrs.domains.career.conftest import make_career_rule
...
class TestCareerOutcomeTaxonomy:
    """Tests for the CareerOutcomeTaxonomy enum."""

    def test_all_outcomes_have_string_values(self) -> None:
        for o in CareerOutcomeTaxonomy:
            assert isinstance(o.value, str)
            assert o.value == o.name

    def test_outcome_count(self) -> None:
        assert len(CareerOutcomeTaxonomy) == 14

    def test_outcome_from_value(self) -> None:
        assert CareerOutcomeTaxonomy("CAREER_ASCENT") is CareerOutcomeTaxonomy.CAREER_ASCENT
        assert CareerOutcomeTaxonomy("GOVERNMENT_SERVICE") is CareerOutcomeTaxonomy.GOVERNMENT_SERVICE

    def test_invalid_outcome(self) -> None:
        with pytest.raises(ValueError):
            CareerOutcomeTaxonomy("INVALID")


class TestCareerRule:
    """Tests for the CareerRule model."""

    def test_creation(self) -> None:
        rule = make_career_rule(rule_id="R-TEST")
        assert rule.rule_id == "R-TEST"
        assert rule.outcome is CareerOutcomeTaxonomy.CAREER_ASCENT

    def test_frozen(self) -> None:
        rule = make_career_rule()
    ...
    def test_to_dict(self) -> None:
        ...
        assert d["outcome"] == "GOVERNMENT_SERVICE"
...
class TestCareerRuleCatalog:
    """Tests for the CareerRuleCatalog model."""

    def test_creation(self) -> None:
        catalog = CareerRuleCatalog(rules=(make_career_rule(),))
        assert len(catalog.rules) == 1

    def test_get_rules_by_outcome(self, sample_catalog: CareerRuleCatalog) -> None:
        ascent = sample_catalog.get_rules_by_outcome(CareerOutcomeTaxonomy.CAREER_ASCENT)
        assert len(ascent) == 1

    def test_get_rule_by_id(self, sample_catalog: CareerRuleCatalog) -> None:
        rule = sample_catalog.get_rule_by_id("R-TEST-002")
        assert rule is not None
        assert rule.rule_id == "R-TEST-002"

    def test_get_rule_by_id_not_found(self, sample_catalog: CareerRuleCatalog) -> None:
        rule = sample_catalog.get_rule_by_id("R-NONE")
        assert rule is None

    def test_to_dict(self, sample_catalog: CareerRuleCatalog) -> None:
...
class TestEvaluateRule:
    ...
    def test_single_condition_match(self) -> None:
        rule = make_career_rule(condition_facts=("flag_a=true",))
        record = evaluate_rule(rule, {"flag_a": True})
        assert record is not None
        assert isinstance(record, EvidenceRecord)
        assert record.outcome_taxonomy == CareerOutcomeTaxonomy.CAREER_ASCENT.value
    ...
    def test_multiple_conditions_all_match(self) -> None:
        rule = make_career_rule(condition_facts=("a=true", "b=true"))
    ...
    def test_empty_conditions_returns_none(self) -> None:
        rule = make_career_rule(condition_facts=())
        record = evaluate_rule(rule, {"flag_a": True})
        assert record is None
...
class TestEvaluateFacts:
    ...
    def test_multiple_rules(self, sample_rules: tuple[CareerRule, ...]) -> None:
        facts = {
            "10th_lord_in_kendra_or_trikona": True,
            "10th_lord_debilitated": True,
        }
        records = evaluate_facts(sample_rules, facts)
        assert len(records) >= 2

    def test_no_rules_match(self, sample_rules: tuple[CareerRule, ...]) -> None:
        records = evaluate_facts(sample_rules, {})
        assert records == ()

    def test_deterministic_output(self, sample_rules: tuple[CareerRule, ...]) -> None:
    ...
    def test_no_false_cross_contamination(self) -> None:
        """Ensure a 'business' rule doesn't trigger 'government service'."""
        rules = (
            make_career_rule(
                rule_id="R-BIZ",
            ...
            make_career_rule(
                rule_id="R-GOV",
                outcome=CareerOutcomeTaxonomy.GOVERNMENT_SERVICE,
                condition_facts=("sun_10th_connection=true",),
            ),
        )
        records = evaluate_facts(rules, {"mercury_strong": True})
        assert len(records) == 1
        assert records[0].outcome_taxonomy == CareerOutcomeTaxonomy.SUCCESSFUL_BUSINESS.value
    ...
    def test_numeric_comparison(self) -> None:
        rules = (
            make_career_rule(
                ...
                condition_facts=("saturn_bala>6.0",),
            ),
        )
        records = evaluate_facts(rules, {"saturn_bala": 7.5})
        assert len(records) == 1


class TestCareerConfig:
    """Tests for the CareerConfig model."""
